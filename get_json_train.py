...
try:
    with tqdm.tqdm(range(len(os.listdir(imgs_path1)))) as tqdm_range:
        for _,img_name in zip(tqdm_range,os.listdir(imgs_path1)):
            ...
            # image,contours, _ = cv2.findContours(img1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  #Open CV 3
            contours, _ = cv2.findContours(img1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  #Open CV 2 & 4
            C = []
            try:
