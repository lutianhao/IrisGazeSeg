...
import torch
from lib.datasets.collate_batch import IrisGazeSeg_collator
...
class Dataset(data.Dataset):
    ...
    def prepare_rcnn(self, abox, instance, cp_hm, cp_wh, cp_ind):
        ...
        hm = np.zeros([1, IrisGazeSeg_config.cp_h, IrisGazeSeg_config.cp_w], dtype=np.float32)
        abox_w, abox_h = abox[2] - abox[0], abox[3] - abox[1]
        cp_wh_ = []
        cp_ind_ = []
        ratio = [IrisGazeSeg_config.cp_w, IrisGazeSeg_config.cp_h] / np.array([abox_w, abox_h])
        ...
        for ex in instance:
            ...
            shift = center - abox[:2]
            ro_center = shift / [abox_w, abox_h] * [IrisGazeSeg_config.cp_w, IrisGazeSeg_config.cp_h]
            ...
            center = ro_center / [IrisGazeSeg_config.cp_w, IrisGazeSeg_config.cp_h] * [abox_w, abox_h] + abox[:2]
    ...
    def prepare_init(self, box, extreme_point, i_it_4pys, c_it_4pys, i_gt_4pys, c_gt_4pys, h, w):
        ...
        img_init_poly = snake_cityscapes_utils.uniformsample(img_init_poly, IrisGazeSeg_config.init_poly_num)
    ...
    def prepare_evolution(self, poly, extreme_point, img_init_polys, can_init_polys, img_gt_polys, can_gt_polys):
        ...
        img_init_poly = snake_cityscapes_utils.uniformsample(octagon, IrisGazeSeg_config.poly_num)
        can_init_poly = snake_cityscapes_utils.img_poly_to_can_poly(img_init_poly, x_min, y_min, x_max, y_max)

        img_gt_poly = snake_cityscapes_utils.uniformsample(poly, len(poly) * IrisGazeSeg_config.gt_poly_num)
    ...
    def get_mask_img(self,path):
        seg_path=path.replace('bp','SegmentationClass')
        seg_path=seg_path.split('.')[0]+'.png'
        # img = cv2.imread(seg_path, -1)
        return seg_path
    ...
    def __getitem__(self, index):
        ...
        input_w, input_h = IrisGazeSeg_config.input_w, IrisGazeSeg_config.input_h
        seg_path=self.get_mask_img(path)
        _target = Image.open(seg_path)
        _target = _target.resize((input_w,input_h), Image.NEAREST)
        # print(_target)
        # _target = np.array(_target, dtype=np.int32)
        _target = np.array(_target, dtype=np.uint8)
        # print(_target.tolist())
        # _target[_target < 128] = 0
        # _target[_target >= 128] = 1
        _target[_target == 255] = 1
        label_img = Image.fromarray(_target)
        # print(path)
        image_name=seg_path.split('/')[-1]
        # label_img.save('samples/{}'.format(image_name))

        seg_img = torch.from_numpy(_target).float()
        ...
        orig_img, inp, trans_input, trans_output, flipped, center, scale, inp_out_hw = \
            snake_cityscapes_utils.augment(
                img, self.split,
                IrisGazeSeg_config.data_rng, IrisGazeSeg_config.eig_val, IrisGazeSeg_config.eig_vec,
                IrisGazeSeg_config.mean, IrisGazeSeg_config.std, instance_polys
            )
        # print(inp)
        ...
        ret = {'inp': inp,'mask':seg_img,'image_name':image_name}
...
if __name__ == '__main__':
    # anfile = r'/root/deep_snake/New_bp/Anno/trian'
    # data_root = r'/root/deep_snake/New_bp/leftImg8bit'
    # split = 'train'
    # dataset = Dataset(ann_file=anfile,data_root=data_root,split=split)
    # data_loader = torch.utils.data.DataLoader(
    #     dataset,
    #     batch_size=6,
    #     collate_fn=IrisGazeSeg_collator,
    #     pin_memory=True,
    #     drop_last=True,
    #     num_workers=6
    # )
    ...
    #     print(i)
    anfile = r'/tmp/Anno/trian'
    data_root = r'/tmp/leftImg8bit'
    split = 'train'
    dataset = Dataset(ann_file=anfile,data_root=data_root,split=split)
    data_loader = torch.utils.data.DataLoader(
        dataset,
