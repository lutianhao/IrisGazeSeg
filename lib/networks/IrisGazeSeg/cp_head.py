...
from lib.utils.IrisGazeSeg import IrisGazeSeg_config, IrisGazeSeg_utils
import torch
...
class ComponentDetection(nn.Module):
    def __init__(self):
        ...
        self.pooler = ROIAlign((IrisGazeSeg_config.roi_h, IrisGazeSeg_config.roi_w))
    ...
    def prepare_training(self, cnn_feature, output, batch):
        ...
        bboxes = torch.cat([xs - wh[..., 0:1] / 2,
                            ...
                            xs + wh[..., 0:1] / 2,
                            ys + wh[..., 1:2] / 2], dim=2)
        rois = IrisGazeSeg_utils.box_to_roi(bboxes, batch['act_01'].byte().bool())
    ...
    def nms_class_box(self, box, score, cls, cls_num):
        ...
        for j in range(cls_num):
            ...
            box_ = box[ind]
            score_ = score[ind]
            ind = _ext.nms(box_, score_, IrisGazeSeg_config.max_ct_overlap)

            box_ = box_[ind]
            score_ = score_[ind]

            ind = score_ > IrisGazeSeg_config.ct_score
            box_ = box_[ind]
            score_ = score_[ind]
            label_ = torch.full([len(box_)], j,dtype=torch.float).to(box_.device)
    ...
    def nms_abox(self, output):
        ...
        ind = torch.cat([torch.full([len(box[i])], i,dtype=torch.float) for i in range(len(box))], dim=0)
    ...
    def prepare_testing(self, cnn_feature, output):
        if IrisGazeSeg_config.nms_ct:
            detection, ind = self.nms_abox(output)
        else:
            ind = output['detection'][..., 4] > IrisGazeSeg_config.ct_score
    ...
    def decode_cp_detection(self, cp_hm, cp_wh, output):
        ...
        ind = output['roi_ind']
        box, cp_ind = IrisGazeSeg_utils.decode_cp_detection(torch.sigmoid(cp_hm), cp_wh, abox, adet)
