...
from lib.utils.IrisGazeSeg import IrisGazeSeg_config
...
def box_to_roi(box, box_01):
    """ box: [b, n, 4] """
    box = box[box_01]
    ind = torch.cat([torch.full([box_01[i].sum()], i,dtype=torch.float) for i in range(len(box_01))], dim=0)
...
def decode_cp_detection(cp_hm, cp_wh, abox, adet):
    ...
    if IrisGazeSeg_config.cp_hm_nms:
        cp_hm = nms(cp_hm)

    abox_w, abox_h = abox[..., 2] - abox[..., 0], abox[..., 3] - abox[..., 1]

    scores, inds, clses, ys, xs = topk(cp_hm, IrisGazeSeg_config.max_cp_det)
    cp_wh = transpose_and_gather_feat(cp_wh, inds)
    cp_wh = cp_wh.view(batch, IrisGazeSeg_config.max_cp_det, 2)
    ...
    for i in range(ct_num):
        cp_ind = _ext.nms(boxes[i], scores[i], IrisGazeSeg_config.max_cp_overlap)
        cp_01 = scores[i][cp_ind] > IrisGazeSeg_config.cp_score
        boxes_.append(boxes[i][cp_ind][cp_01])

    cp_ind = torch.cat([torch.full([len(boxes_[i])], i,dtype=torch.float) for i in range(len(boxes_))], dim=0)
