...
def get_box_match_ind(pred_box, score, gt_poly):
    ...
    # tmp_box_iou = snake_config.box_iou
    # tmp_comfidence = snake_config.confidence
    # while True:
    box_ind = ((iou > snake_config.box_iou) * (score > snake_config.confidence)).nonzero().view(-1)
    #     tmp_box_iou +=0.001
    #     tmp_comfidence += 0.001
    #     if len(box_ind) == 2:
    #         break
...
def prepare_training(ret, batch):
    ...
    ct_num = batch['meta']['ct_num']
    init.update({'4py_ind': torch.cat([torch.full([ct_num[i]], i,dtype=torch.float) for i in range(ct_01.size(0))], dim=0)})
